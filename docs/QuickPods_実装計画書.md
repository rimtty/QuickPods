...
| NFR-REL-007 | リリース判断者が承認したエージング期間でGDI、USER、メモリに重大な継続増加がない |
...
- Windows 11が提示する100、125、150、175、200、225、250、300、350%のDPIと混在DPIを試験する。
...
- テストマトリクス実施
- リリース判断者が承認した期間のエージング試験
- Explorer再起動反復
- Bluetooth実機機能証拠の確認（追加の50-cycle耐久は2026-08-07の製品判断で不要）
...
| ケース | 合格条件 |
|---|---|
| 100、125、150、175、200、225、250、300、350% | 表示とヒット位置が一致し、既存UIと重ならない |
...
リリース判断者が承認した観測時間（0.1.0では2.01時間）稼働させ、次を監視する。
...
- AC-025：承認されたエージング試験で重大な継続的リソース増加がない。
...
- 受入基準を満たす。
- 承認されたエージング試験とExplorer再起動反復を通過する。
