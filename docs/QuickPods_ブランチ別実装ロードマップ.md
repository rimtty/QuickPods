...
| Remote | `origin/main`へPhase 0～6Aの実装PRを統合済み。全体進捗はIssue #2、残存Gateは#15／#18／#19／#33／#34／#41／#43／#48／#50で管理。物理catalog #38とPhase 5C #53は完了済み |
...
| 実機試験 | 参照機器の有効な接続5回、切断5回、圏外、ケース内、他端末、無線OFF、選択外機器影響 |
...
- 機器が到達可能等の事前条件成立時に、接続・切断がそれぞれ5回中4回以上、15秒以内に実状態で確認できる。
...
| 状態 | **実装・UI手動Gate完了（2026-08-07）** — PR #54をPhase 6BへSquash統合。実pointer anchor、hover dismiss、tray-first起動、Start 10秒超continuity、Release build、380件回帰、ユーザー目視に合格。実Bluetooth一覧#38は合格し、操作#41、local-console DPI／keyboard #43へ継続 |
| 実装 | タスクバー内volume／device表示、non-activating hover、surface anchor、borderless flyout、空一覧、device rows、主操作、設定導線 |
| 境界 | 左揃え非対応policyを維持。物理Bluetooth操作はIssue #41、local-console DPIはIssue #43で判定 |
