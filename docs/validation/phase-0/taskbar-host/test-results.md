...
| `WS_POPUP`可視試験 | **Pass／採用方式** — 100／125%の安定性に加え、100／150%の各120秒実機runでStart／Search表示中も同じnative位置を維持。Floating遷移0、表示中wheel入力、自然終了後残留0。ユーザー目視も合格 |
...
| 検索：アイコン＋ラベル | **Pass（Phase 0E／100／150% Popup）** — Start／Searchをそれぞれ表示した状態でnative位置を保持し、表示中のwheel入力も成功。継続証明が成立し、Floating遷移0 |
...
| 150% | **Pass（採用Popup continuity）** — DPI 144、中央揃え、検索アイコン＋ラベル、Task View／Widgets ON。自動監査はstyle／taskbar parent／ownerなし／DWM／drag／wheel、exit 0、残留0。120秒目視runはnative継続3回、Floating遷移0、wheel 273件、drag開始／完了12／12、fatal 0、自然終了後残留0 |
| ピン留めアプリ多数 | **Pass（150%採用Popup）** — button 27→52、Start X 1919→1094、fault 0、`Place / Standard (440, 6, 450, 60)`。自動監査と75秒手動runに合格。native継続3回、Floating遷移0、wheel 18件、drag開始／完了18／18、move 188件、fatal 0、自然破棄、残留0 |
| Start中tray churn | **Pass（150%採用Popup）** — 通知アイコンを45回追加／45回削除し正常破棄。native継続4回（`DirectExpected` 2回）、Floating遷移0、wheel 67件、drag開始／完了5／5、move 158件、watcher failure／fatal 0、native正常破棄、helper／host残留0。ユーザー目視・入力確認合格 |
...
- 150%・DPI 144・中央揃え・検索アイコン＋ラベル・Task View／Widgets ONで採用Popupを120秒実行した。StartとSearchを各10～15秒表示しても同じタスクバー位置を目視維持し、表示中wheel入力に成功した。ログはnative継続3回（`DirectExpected`を含む）、Floating遷移0、wheel 273件、drag開始／完了12／12、fatal 0、`native-host=destroyed`、終了後プロセス残留0だった。事前の12秒自動監査もPopup style、実親、ownerなし、non-topmost、DWM uncloaked、drag／wheel、exit 0、残留0に合格した。
- 150%のままユーザーがタスクバーへ多数のアプリを追加した。読み取り専用探索はUIA button 27→52、Start X 1919→1094、fault 0、`Place / Standard (440, 6, 450, 60)`となった。12秒自動監査はPopup style、実親、ownerなし、non-topmost、DWM、drag／wheel、exit 0、残留0に合格した。75秒手動runはnative継続3回、Floating遷移0、wheel 18件、drag開始／完了18／18、move 188件、fatal 0、正常破棄、残留0で、ユーザーは多数アイコンとの重なり、ちらつき、click／drag／wheelすべて問題なしと確認した。一時UI中のbutton 1観測から52件へ安全に復帰した。
- Startを開いたまま、専用診断helperが通知アイコンを500ms間隔で45回追加／45回削除した。helperは`disposed=true`、stderrなし、既知PID残留0だった。採用Popupは120秒runで`Starting → NativeVisible`だけを維持し、native継続4回（`DirectExpected` 2回）、Floating遷移0、wheel 67件、drag開始／完了5／5、move 158件、watcher failure／fatal 0、`native-host=destroyed`、終了後プロセス残留0だった。ユーザーはStart表示中のタスクバー内保持、ちらつき、重なり、入力に問題なしと確認した。
- 全実機マトリクスと7件のGo条件が合格したためGate Bを**Go**とする。製品実装は別プロセスの`PopupPreserved`を採用し、unsafe／NoFitはfloatingまたはhiddenへfail closedする。Issue #15とIssue #18はリリース前P2として継続する。
...
- このPhase 0Cの150%構成でChild／Popupを各30秒実行した。独立列挙は両方式とも実行中View／Control各1、最大各1、重複0、exit 0、終了後0だった。当時はChildのドラッグとホイール、静止画だけを確認し、Popupの目視・入力所見は未回答だった。採用Popupの150%目視は上記Phase 0E runで後に完了した。
...
Childの150%での静止画とドラッグ／ホイール、および125%両方式の入力ログは確認済みで、Issue #12はClose済みである。100%の検索4形式、Task View／Widgets ON検索ボックス、100／200%左揃えNoFit floatingは自動・手動確認と終了後残留0に合格し、Issue #11の受入条件を満たした。Phase 0Eでは100／150% PopupでStart／Search双方のnative continuityと表示中wheel入力、多数ピン留めstress、Start中tray churnに合格し、PopupPreservedを最終方式に選定した。採用PopupのExplorer再起動10回も機能要件に合格し、UIA watcherの世代別資源寿命はIssue #18へ分離した。Gate BはGoである。
