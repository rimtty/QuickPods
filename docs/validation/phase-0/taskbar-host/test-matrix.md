...
2026-08-05のPhase 0E Release試験ではTaskbarHost 364件とsolution smoke 1件、計365件が合格した。Release buildは0 warning／0 error、formatとdiff checkも合格した。Phase 0Dの301件＋1件とPhase 0Cの216件＋1件はhistorical baselineとして保持する。非interactive環境ではnative HWND試験を成功扱いで素通りさせず、明示的Skipとして報告する。
...
| 検索：アイコン＋ラベル | 初期配置と一時UIからの復帰 | **Pass（Phase 0E／100% Popup）** — 初期`Place / Standard`、Start／Search表示中もnative位置を保持し、表示中wheel入力と終了後残留0に合格 |
...
| Start／検索の一時UI | native fail-closedを維持し、同一generationを直接再証明できる場合だけnativeを保持 | **Pass（Phase 0E／100% Popup）** — `DirectExpected`、retained notification／Start、direct host attachmentを再証明し、120秒runでFloating遷移0。ユーザーは両UI中のnative保持を確認 |
...
| DPI 150% | attach前後でDPI 144を維持し、描画と入力位置が一致 | **Pass（基礎attach／Child入力）** — Childで描画、ドラッグ、ホイールを実機確認し、Child／Popupの自動attachと終了後残留0を確認。Popup採用後のStart／Search continuityは残るDPI matrix項目 |
...
| DPI 100／125% | 描画と入力位置が一致 | **Pass（試験済みPlace構成）** — 125%両styleと100%の検索4形式に合格。100% Popupは一時UI native continuityにも合格。100%左揃えはNoFitでFail Closed |
...
Issue #12のちらつきとIssue #13のStart／Search native continuityは実機確認まで合格し、PopupPreservedを採用方式に選定した。provenance／raceのP2強化はIssue #15で追跡する。採用PopupでのExplorer再起動10回、ピン留めアプリ多数、tray churn中のStart保持、および残るDPI／fallback目視項目は未完了であるため、Gate B全体はPendingのままとする。
