...
QuickPodsは、Windowsでペアリング済みのBluetoothオーディオ機器を一覧表示し、利用者が明示的に1台を選択して接続または切断できるようにする。接続完了後は、その機器の再生エンドポイントをWindowsの既定出力へ設定し、以前接続されていた別のBluetoothオーディオを切断して音声接続を1台へ収束させる。対象はヘッドホン、ヘッドセット、イヤホン、スピーカー等であり、AirPodsは互換性を確認する参照機器の1つとして扱う。
...
3. 接続・切断要求は、選択機器または切替前に接続済みと確認したBluetoothオーディオのContainer IDに属する検証済みKSフィルターだけへ送信する。キーボード、マウス、コントローラー等の非オーディオ機器は対象にしない。
...
- 表示順は接続済み、表示名の順を基本とし、同順位では安定した内部順を用いる。選択変更だけでは並び順を変えない。

### 3.2 表示情報

各行は表示名、一般化した機器種別アイコン、`ペアリング済み`、接続状態、既定出力状態を持つ。状態は`接続済み・既定`、`接続済み`、`未接続`、`接続中`、`既定の出力へ切替中`、`前の機器を切断中`、`切断中`、`利用不可`、`状態不明`、`直接操作は未対応`に正規化する。
...
- 再起動後の初期選択は現在の実状態から決定する。接続済みかつ既定出力の機器、最初の接続済み機器、表示順の先頭の順で優先する。
- 起動時の初期選択と通常の選択変更では、自動接続、切断、既定出力変更を行わない。
- 選択機器が消失した場合は`利用不可`として再列挙を待つ。再ペアリングでIDが変わった場合は再選択を案内する。
- 起動後の通常更新では利用者の選択を維持し、再検索や外部状態通知だけで選択行を変更しない。
...
7. 接続済みと既定出力の両方が確認できた後、操作開始時に接続済みだった別のBluetoothオーディオへContainer単位のDisconnectを順番に送信し、非Activeを確認する。
8. 新しい機器の接続または既定化に失敗した場合は旧機器を切断しない。旧機器の直接操作が非対応または切断失敗の場合は、新しい機器への切替成功を保持したまま警告を表示する。
...
明示的な切断時は選択ContainerのDisconnectだけを送信し、非Activeを確認する。接続切替後の自動切断は、操作開始時に接続済みだった別のBluetoothオーディオContainerだけを対象にする。Windowsが別の既定出力を自動選択したことを追跡するが、QuickPodsが過去の既定出力へ強制復元することは初期仕様に含めない。
...
4. Reconnectと既定出力指定は選択Containerだけへ送り、自動切断は操作開始時に接続済みだった別のBluetoothオーディオContainerだけへ送ること。非オーディオBluetooth機器への要求は0件であること。
...
9. 再起動時に接続済み・既定、接続済み、表示順の先頭を優先して初期選択し、変更要求を送らないこと。再ペアリング後は安全な再選択を案内すること。
10. キーボード、スクリーンリーダー、高コントラスト、200% DPI。
11. 参照実機では到達可能前提を確認した接続・切断各5回中4回以上、非オーディオBluetooth機器への影響0件。接続／切断／既定化／旧オーディオの自動切断／実音声／外部状態追従の実機証拠を十分とし、追加の長時間反復は2026-08-07の製品判断で要求しない。
